...
public class AntForestRpcCall {
    ...
    public static void init() {
        if (AlipayInfo.compareVersionName("10.5.88.8000") > 0) {
            VERSION = "20240403";
        } else if (AlipayInfo.compareVersionName("10.3.96.8100") > 0) {
            VERSION = "20230501";
        } else {
            VERSION = "20230501";
        }
    ...
    public static String antiepSign(String entityId, String userId,String sceneCode) {
        return ApplicationHook.requestString("com.alipay.antiep.sign",
                "[{\"entityId\":\"" + entityId
                        + "\",\"requestType\":\"rpc\",\"sceneCode\":\""+sceneCode+"\",\"source\":\"ANTFOREST\",\"userId\":\""
                        + userId + "\"}]");
    }

    public static String queryPropList(boolean onlyGive) {
        String args = "[{\"onlyGive\":\"" + (onlyGive ? "Y" : "") + "\",\"source\":\"chInfo_ch_appcenter__chsub_9patch\",\"version\":\"" + VERSION + "\"}]";
        return ApplicationHook.requestString("alipay.antforest.forest.h5.queryPropList", args);
    }
    ...
    public static String giveProp(String giveConfigId, String propId, String targetUserId) {
        // 从 RuntimeInfo 中读取 rdsBizNo 和 rdsToken
        String rdsBizNo = RuntimeInfo.getInstance().getString(RuntimeInfo.RuntimeInfoKey.ForestGivePropRdsBizNo);
        String rdsToken = RuntimeInfo.getInstance().getString(RuntimeInfo.RuntimeInfoKey.ForestGivePropRdsToken);
        String args = "\"giveConfigId\":\"" + giveConfigId + "\",\"propId\":\"" + propId + "\",\"source\":\"friend_btn\",\"targetUserId\":\"" + targetUserId + "\"";
        if (!StringUtil.isEmpty(rdsBizNo) && !StringUtil.isEmpty(rdsToken)) {
            args += ",\"rdsBizNo\":\"" + rdsBizNo + "\",\"rdsToken\":\"" + rdsToken + "\"";
        }
        return ApplicationHook.requestString("alipay.antforest.forest.h5.giveProp", "[{" + args + "}]");
    }

    public static String collectProp(String giveConfigId, String giveId) {
        String args = "[{\"giveConfigId\":\"" + giveConfigId + "\",\"giveId\":\"" + giveId + "\",\"source\":\"chInfo_ch_appcenter__chsub_9patch\"}]";
        return ApplicationHook.requestString("alipay.antforest.forest.h5.collectProp", args);
    }

    public static String consumeProp(String propId, String propType) {
        String args = "[{\"propId\":\"" + propId + "\",\"propType\":\"" + propType + "\",\"source\":\"chInfo_ch_appcenter__chsub_9patch\"}]";
        return ApplicationHook.requestString("alipay.antforest.forest.h5.consumeProp", args);
    }
    ...
    public static String exchangeBenefit(String spuId, String skuId) {
        String args = "[{\"sceneCode\":\"ANTFOREST_VITALITY\",\"requestId\":\""
                + System.currentTimeMillis() + "_" + RandomUtil.getRandom(16)
                + "\",\"spuId\":\"" + spuId + "\",\"skuId\":\"" + skuId + "\",\"source\":\"GOOD_DETAIL\"}]";
    ...
    // 森林皮肤
    public static String getIndexDress() {
        return ApplicationHook.requestString("alipay.antforest.forest.h5.getIndexDress", "[{}]");
    }
    ...
    public static String checkUserSecondSceneChance() {
        return ApplicationHook.requestString("com.alipay.creditapollon.biz.rpc.api.rent.green.checkUserSecondSceneChance",
                "[{\"requestSceneCode\":\"rentBrowse\"}]");
    }
