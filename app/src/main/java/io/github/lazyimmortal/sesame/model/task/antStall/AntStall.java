...
//        taskTypeList.add("ANTSTALL_XLIGHT_VARIABLE_AWARD");
    ...
    private synchronized void inviteOpenShop(String seatId) {
        ...
        try {
            ...
            for (int i = 0; i < friendRankList.length(); i++) {
                ...
                boolean isInviteShop = inviteOpenShopList.contains(friendUserId);
            ...
            for (int i = 1; i <= 2; i++) {
                ...
                if (System.currentTimeMillis() > endTime) {
                    ...
                    if (!hasChildTask(taskId)) {
                        ...
                        Log.record("添加蹲点请走⛪在[" + TimeUtil.getCommonDateTime(endTime) + "]执行");
                    } /*else {
                        addChildTask(new ChildModelTask(taskId, "SB", () -> {
                    ...
                    if (System.currentTimeMillis() > shopTime) {
                        ...
                        if (!hasChildTask(taskId)) {
                            addChildTask(new ChildModelTask(taskId, "SH", () -> {
                                if (closeShop(shopId, rentLastBill, rentLastUser)) {
                                    TimeUtil.sleep(300L);
                                    if (openShopType.getValue() != OpenShopType.NONE) {
                                        openShop();
                                    }
                            ...
                            Log.record("添加蹲点收摊⛪在[" + TimeUtil.getCommonDateTime(shopTime) + "]执行");
                        }
                    }
                }
            }
        ...
        try {
            ...
            for (int i = 0; i < friendRankList.length(); i++) {
                ...
                if (friendRank.getBoolean("canOpenShop")) {
                    ...
                    boolean isStallOpen = openShopList.contains(userId);
    ...
    private Boolean  closeShop(String shopId, String billNo, String userId) {
        try {
    ...
    private static void receiveTaskAward(String taskType, String title) {
        try {
            ...
            if (MessageUtil.checkResponse(TAG, jo)) {
                Log.farm("新村任务🎖️领取奖励[" + title + "]#产速增加" + jo.getInt("incAwardCount") + "/小时");
            }
        } catch (Throwable t) {
    ...
    private void inviteRegister() {
        ...
        try {
            ...
            for (int i = 0; i < friendRankList.length(); i++) {
                ...
                if (!inviteRegisterList.contains(userId)) {
                    continue;
                }
    ...
    private String shareP2P() {
        try {
            JSONObject jo = new JSONObject(AntStallRpcCall.shareP2P());
            if (MessageUtil.checkResponse(TAG, jo)) {
    ...
    private void assistFriend() {
        try {
            if (Status.hasFlagToday(AntStallFlag.SHARE_P2P_LIMIT.flagName())) {
                return;
            }
            for (String friendUserId : assistFriendList.getValue()) {
                ...
                TimeUtil.sleep(5000);
                if (MessageUtil.checkResponse(TAG, jo)) {
                    Log.farm("新村助力🎉成功助力[" + UserIdMap.getMaskName(friendUserId) + "]");
                    Status.stallShareP2PToday(friendUserId);
                } else if (Objects.equals("600000027", jo.getString("code"))) {
                    Status.flagToday(AntStallFlag.SHARE_P2P_LIMIT.flagName());
                    return;
                } else {
                    Status.flagToday(AntStallFlag.SHARE_P2P_LIMIT.flagName(friendUserId));
                }
    ...
    private Boolean canDonateToday() {
        if (Status.hasFlagToday(AntStallFlag.DONATE.flagName())) {
            return false;
        }
        try {
            ...
            Status.flagToday(AntStallFlag.DONATE.flagName());
        } catch (Throwable t) {
    ...
    private Boolean unlockNewVillage() {
        try {
            ...
            Log.farm("蚂蚁新村⛪解锁新村[" + villageName + "]#" + villageDesc);
            return true;
        } catch (Throwable t) {
    ...
    private Boolean throwManure(JSONArray dynamicList) {
        try {
            ...
            if (MessageUtil.checkResponse(TAG, jo)) {
                ...
                Status.flagToday(AntStallFlag.THROW_MANURE_LIMIT.flagName());
            }
        } catch (Throwable th) {
    ...
    private void throwManure() {
        if (Status.hasFlagToday(AntStallFlag.THROW_MANURE_LIMIT.flagName())) {
            return;
        }
        try {
            ...
            for (int i = 0; i < astLossDynamicVOS.length(); i++) {
                ...
                boolean isThrowManure = throwManureList.contains(objectId);
    ...
    private void pasteTicket() {
        if (Status.hasFlagToday(AntStallFlag.PASTE_TICKET_LIMIT.flagName())) {
            return;
        }
        try {
            while (true) {
                ...
                if (jo.getInt("canPasteTicketCount") == 0) {
                    ...
                    Status.flagToday(AntStallFlag.PASTE_TICKET_LIMIT.flagName());
                    return;
                }
    ...
    private void pasteTicket(String friendUserId) {
        boolean isStallTicket = pasteTicketList.contains(friendUserId);
    ...
    public enum AntStallFlag implements Status.StatusFlag {
        DONATE,
        SHARE_P2P_LIMIT,
        THROW_MANURE_LIMIT,
        PASTE_TICKET_LIMIT
